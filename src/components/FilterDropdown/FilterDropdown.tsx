...
const FilterDropdown: FC = () => {
  ...
  return (
    <div className="relative w-full max-w-[280px] group text-xs z-50">
      <button
        type="button"
        className="relative btn btn-sm w-auto text-[10px] bg-transparent hover:bg-transparent border-light hover:border-white text-gray-500 inline-flex items-center justify-between gap-1 after:content-['\f0d7'] after:relative after:font-awesome after:font-[900]"
      >
        Select View / Filter Type <span className="mx-auto">|</span> {value}
      </button>
      <ul className="absolute left-0 top-100 mt-2 w-full max-w-[220px] text-start p-3 bg-light rounded-md shadow-full-match invisible opacity-0 group-focus-within:visible group-focus-within:opacity-100 transition-all duration-300">
        <li
